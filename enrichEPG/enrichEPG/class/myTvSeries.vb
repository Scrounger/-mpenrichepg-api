...
#Region "Member"
    ...
    Private Shared _database As String = "C:\ProgramData\Team MediaPortal\MediaPortal\database\TVSeriesDatabase4.db3"
    Private Shared _onLineSeriesCoulumns As String = "ID, Pretty_Name, origName, PosterBannerFileName, Summary, Rating, Network, Status"
    Private Shared _SqlEpisodeConstructor As String = String.Format("Select {0} FROM online_series", _onLineSeriesCoulumns)
#End Region
    ...
    Public Property EpisodesList() As IList(Of MyTvSeries.MyEpisode)
        Get
            Select Case (m_EpisodesList.Count)
                Case Is = 0
                    Return MyEpisode.ListAll(idSeries)
                Case Else
...
#Region "Functions"
    ...
    ''' </summary>
    Public Function GetFanArt() As String
        Dim _SqlString As String = String.Format("Select LocalPath FROM Fanart WHERE seriesID = '{0}' AND LocalPath LIKE '_%'", idSeries)
        ...
        Return _Result.GetField(0, 0)
    End Function
#End Region

#Region "SubS"
    Public Shared Sub SetMPdatabasePath(ByVal MpDbPath As String)
        _database = MpDbPath
    End Sub
#End Region
...
#Region "Retrieval"
    ...
    Public Shared Function Search(ByVal SeriesName As String) As MyTvSeries
        Dim _SqlString As String = String.Format("{0} WHERE Pretty_Name LIKE '{1}' OR SortName LIKE '{1}' OR origName LIKE '{1}'", _
                                _SqlEpisodeConstructor, Helper.allowedSigns(SeriesName))
        Try
            Return Helper.GetSeries(_SqlString).Item(0)
        Catch ex As Exception
            ...
            For Each _Series In _SeriesList
                If Helper.levenshtein(SeriesName, _Series.Title) <= 2 Then
                    Return _Series
                End If
            Next

            'Falls noch immer nicht gefunden, prüfen ob "-" in SerienName, danach alles abschneiden und levenshtein
...
#Region "Retrieval"
        ...
        Public Shared Function Search(ByVal idSeries As Integer, ByVal EpisodeName As String) As MyTvSeries.MyEpisode
            ...
            Try
                Return Helper.GetEpisodes(_SqlString).Item(0)
            Catch ex As Exception
                Dim _EpisodeList As IList(Of MyTvSeries.MyEpisode) = MyTvSeries.MyEpisode.ListAll(idSeries)

                For Each _episode In _EpisodeList
                    Dim _variance As Integer = Helper.levenshtein(EpisodeName, _episode.EpisodeName)
                    If _variance <= 2 Then
                        Helper.logLevenstein = String.Format(", variance = {0}, TvSeriesDB: {1}", _variance, _episode.EpisodeName)
                        Return _episode
                    End If
                Next
            ...
            Try
                ...
                If m_db IsNot Nothing Then
                    Try
                        m_db.Close()
                        m_db.Dispose()
                        MyLog.Debug("[MyTvSeries]: [OpenTvSeriesDB]: Disposing current instance..")
                ...
                If File.Exists(_database & "\TVSeriesDatabase4.db3") = True Then

                    m_db = New SQLiteClient(_database & "\TVSeriesDatabase4.db3")
                ...
                Else
                    MyLog.Error("[MyTvSeries]: [OpenTvSeriesDB]: TvSeries Database not found: {0}", _database & "\TVSeriesDatabase4.db3")
                End If

            Catch ex As Exception
                MyLog.Error("[MyTvSeries]: [OpenTvSeriesDB]: exception err:{0} stack:{1}", ex.Message, ex.StackTrace)
                OpenTvSeriesDB()
            End Try
